...
manual_root=${RIGID_RESCUE_MANUAL_ROOT:-${run_dir}}
out_root=${RIGID_RESCUE_ROOT:-${repo_root}/tmp/native_sn_rigid_rescue_$(date -u +%Y%m%dT%H%M%SZ)}
chunk_count=${RIGID_RESCUE_CHUNK_COUNT:-4}
runner=${repo_root}/Scripts/run_native_sn_rigid_rescue_chunk.sh
...
printf "chunk\tjobid\tcount\tsubjects_file\toutput_root\n" > "${summary}"
...
while (( i < ${#subjects[@]} )); do
  ...
  job_name="rigid_sn_${tag}"
  jid=$(sbatch --parsable --job-name="${job_name}" \
    --output="${chunk_dir}/${job_name}.out" --error="${chunk_dir}/${job_name}.err" \
    --export=ALL,RIGID_RESCUE_SUBJECTS_FILE="${subjects_file}",RIGID_RESCUE_MANUAL_DIR="${manual_dir}",RIGID_RESCUE_INPUT_DIR="${input_dir}",RIGID_RESCUE_OUTPUT_ROOT="${chunk_dir}" \
    "${runner}")
  printf "%s\t%s\t%s\t%s\t%s\n" "${tag}" "${jid}" "$(wc -l < "${subjects_file}")" "${subjects_file}" "${chunk_dir}" >> "${summary}"
  i=$(( i + chunk_size ))
  chunk=$(( chunk + 1 ))
done
