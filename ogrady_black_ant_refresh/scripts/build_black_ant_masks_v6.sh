...
# build_black_ant_masks_v6.sh
# -----------------------------------------------------------------------------
# PURPOSE
#   Refresh the MOTIP subject-space neuromelanin probability masks that feed
#   the Black ANT metrics pipeline. V6 starts from the stable V4 workflow,
#   keeps the proven template→anat→TSE chain, and folds in the extras we added
#   for the recent reruns (module-aware AFNI setup, versioned output folders,
#   staged brainstem/DC masks, and manifest logging) without the brittle
#   mask_voxel_count bookkeeping that broke job 385.
#
# WORKFLOW OVERVIEW
...
out_dir=${BLACKANT_OUTPUT_DIR:-${snheart_root}/TSE/probabilistic_masks_v6}
mkdir -p "${out_dir}"
atlas_dir=${BLACKANT_ATLAS_DIR:-${snheart_root}/ROIs/black_ant_templates_v6}
mkdir -p "${atlas_dir}"
manifest_path=${BLACKANT_MASK_MANIFEST:-${out_dir}/black_ant_mask_manifest_v6.tsv}
...
brainstem_dir=${BLACKANT_BRAINSTEM_DIR:-${snheart_root}/TSE/brainstem_masks_v6}
